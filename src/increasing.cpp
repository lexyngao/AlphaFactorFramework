...
double IncreaseSkew::get_value() const {
    if (n < 2) {
        ...
        // 简化实现，避免依赖ComputeUtils
        if (sum_m2 <= 0) return std::numeric_limits<double>::quiet_NaN();
        double g1 = sum_m3 / std::pow(sum_m2, 1.5);
...
double IncreaseKurt::get_value() const {
    if (n < 3) {
        ...
        // 简化实现，避免依赖ComputeUtils
        if (sum_m2 <= 0) return std::numeric_limits<double>::quiet_NaN();
        double g2 = (double(n) * sum_m4) / std::pow(sum_m2, 2) - 3.0;
